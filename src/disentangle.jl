...
function optimize_theta_disentangle(O, G, M; stepsize=.001, verbose=1)
    Op = p_space(O, M)
    GOGp = p_space(G*O*G, M)
    comp = p_space(G*O-O*G, M)
    A = inner_product(Op,Op) 
    B = inner_product(GOGp,GOGp) 
    C = inner_product(comp,comp) 
    D = inner_product(Op,GOGp) 
    E = inner_product(Op,comp) 
    F = inner_product(GOGp,comp) 

    verbose < 2 || @show A, B, C, D, E

    function cost(θ)
        # C(x) = ||P(U(x)' O U(x))||
        #      = ||P((cos(x/2)+isin(x/2)G) O (cos(x/2)-isin(x/2)G)) ||
        #      = ||cos(x/2)^2 P(O) + sin(x/2)^2 P(GOG) + isin(x/2)cos(x/2) P([G,O]))||
        #
        #      = tr(cos(x/2)^2 P(O)' + sin(x/2)^2  P(GOG)' - isin(x/2)cos(x/2) P([G,O])')
        #          *cos(x/2)^2 P(O) + sin(x/2)^2  P(GOG) + isin(x/2)cos(x/2) P([G,O])) )
        #
...
function dbf_disentangle(Oin::PauliSum{N,T}, M::Int;
    ...
    evolve_truncation::TruncationStrategy = CoeffTruncation(1e-12)) where {N,T}
    O = deepcopy(Oin)
    generators = Vector{PauliBasis}([])
    angles = Vector{Float64}([])
    norm_old = norm(q_space(O, M))
